...
foreach ($uploadFiles as $i => $uploadFileName) {
    ...
    $file["control1"] = imagecolorat($image, $file["width"] - 1, $file["height"] - 1);
    $file["control2"] = imagecolorat($image, 1,                  $file["height"] - 2);
    $file["control3"] = imagecolorat($image, $file["width"] - 6, $file["height"] - 3);
    ...
    if (
        ...
        continue;
    } else if ($file["control1"] == 8421504 // Broken image control
            && $file["control2"] == 8421504
            && $file["control3"] == 8421504
   ) {
       continue;
   }
   imagedestroy($image);
...
foreach ($processFiles as $i => $file) {
    ...
    $newFlig1Width  = intval($width / 100 * 20);
    ...
    $flig1y = intval($height - ($newFlig2Height / 2 / 100 * 22) - ($newFlig1Height / 2));
